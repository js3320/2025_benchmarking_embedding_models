...
for metric_name, values in results.items():
    ...
    print(f"{metric_name.upper():>12}: {mean_val:.4f} ± {std_val:.4f} (95% CI: ±{ci_95:.4f})")

avg_cm = np.mean(all_cms, axis=0).astype(int)
print(f"\nAVERAGE CONFUSION MATRIX:")
print(f"Predicted:    0    1")
print(f"Actual 0: {avg_cm[0,0]:4d} {avg_cm[0,1]:4d}")
print(f"Actual 1: {avg_cm[1,0]:4d} {avg_cm[1,1]:4d}")
...
output_dir = '/rds/general/user/js3320/home/(2025_summer)embeddings/codes/MedImageInsights/ATELECTASIS/'  # Change this to your desired path
...
print("="*60)
print(f"Training Strategy: Augmented data for training, original only for testing")
print(f"Training set: {len(X_train)} samples ({train_df['dicom_id'].nunique()} unique IDs)")
print(f"Test set: {len(X_test)} samples ({test_df['dicom_id'].nunique()} unique IDs)")
print(f"No data leakage: ✅ (train and test IDs are completely separate)")
...
print(f"  {'SPECIFICITY':>12}: {spec:.4f}")
...
print(f"\nMODEL BENEFITS:")
print(f"  ✅ Realistic evaluation (no test data leakage)")
print(f"  ✅ Robust training ({len(X_train)} augmented samples)")
print(f"  ✅ Unbiased testing (original images only)")
print("="*60)
